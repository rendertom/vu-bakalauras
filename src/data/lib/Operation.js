class Operation {
  ...
  isSub() {
    return this.operation === 'SUB';
  }
  isMul() {
    return this.operation === 'MUL';
  }
  isDiv() {
    return this.operation === 'DIV';
  }
  ...
  static requiresBorrowing = (num1, num2) => {
    ...
    for (let i = 0; i < Math.min(digits1.length, digits2.length); i++) {
      ...
      if (
        next &&
        next > 1 &&
        parseInt(digits1[i], 10) < parseInt(digits2[i], 10)
      )
        return true;
    }
    return false;
  ...
  static requiresCarrying = (num1, num2) => {
    ...
    const digits2 = num2.toString().split('').reverse();
    for (let i = 0; i < Math.min(digits1.length, digits2.length); i++) {
      if (parseInt(digits1[i], 10) + parseInt(digits2[i], 10) >= 10)
        return true;
    }
    return false;
  };
